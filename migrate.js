...
const connect = function connect() {
  ...
  if (commandMap.has('-url')) {
    ...
    url =
      `mysql://${commandMap.get('-u')}:${commandMap.get('-p')}@` +
...
const updateVersion = function updateVersion() {
  return new Promise((approve, reject) => {
    if (
      versionCompare(currentfolder, lastfolder) >= 0 &&
      versionCompare(currentVersion, lastVersion) >= 0
    )
      return approve();
    consola.info(
      `Previous folder: ${currentfolder}, Last folder: ${lastfolder}`,
    );
    consola.info(
      `Previous version: ${currentVersion}, Last version: ${lastVersion}`,
    );

    consola.success(
      `Updating Folder => ${lastfolder}  DBVersion => ${lastVersion}`,
    );
    return connection.query(
      `UPDATE dbversion SET version = ${lastVersion} , folder=${lastfolder}`,
      (err) => {
        if (err) {
          consola.error(err);
          return reject(err);
...
const getGreaterThanVersionFiles = function getGreaterThanVersionFiles(
  currentF,
  currentV,
) {
...
const run = function run(folder, filename) {
  return new Promise((approve, reject) => {
    const raw = fs
      .readFileSync(`${MIGRATION_ABSOLUTE_PATH}/${folder}/${filename}`)
      .toString();
...
const changeLogTableIfNotExist = function changeLogTableIfNotExist() {
  return new Promise((approve, reject) => {
    const queryChangeLog =
      'CREATE TABLE IF NOT EXISTS dbversion_changelog (' +
...
const dbVersionTableIfNotExist = function dbVersionTableIfNotExist() {
  return new Promise((approve, reject) => {
    const queryChangeLog =
      ...
      '( `id` int(11) NOT NULL auto_increment, `version` varchar(45) DEFAULT NULL, `folder` varchar(45) DEFAULT 0, primary key (id) ) ENGINE=InnoDB DEFAULT CHARSET=utf8';
